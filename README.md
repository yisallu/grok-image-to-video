...
> **Alt + 左键点击**网页上任意图片,调用 xAI **Grok Imagine**(`grok-imagine-video`)生成视频,完成后**在新标签页打开生成的视频**。配套一个轻量代理服务,负责集中放置凭证、绕过超时、处理防盗链图片,并可把成品推送到 Telegram 存档。

---

## ✨ 功能

- **Alt + 左键点图即生成**,完成后**在新标签页打开生成的视频**(后台打开,不打断当前浏览)。
...
浏览器扩展 ──HTTPS──▶  代理服务 /i2v  ──▶  xAI grok-imagine-video  ──▶ 视频 URL
  (Alt+点图)            鉴权/取图/                (异步:提交→轮询)          │
                        轮询/推Telegram                                      ▼
 新标签打开 ◀──────────  /status 轮询  ◀────────────────────────  done + video.url
```
...
├── content.js           # Alt+点击监听、canvas 取本地/同源图、进度遮罩、新标签打开、排队
...
- **图片**:网页上 **按住 Alt + 左键点击** 任意图片 → 等待(约 1–3 分钟)→ 生成的视频在新标签页打开。
...
- **MV3 特性**:生成是后台异步,关掉标签页不影响 Telegram 推送;但页面端的「新标签打开」依赖发起标签页存活到生成完成。
