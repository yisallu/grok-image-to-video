...
- **视频关键帧续写**:**暂停任意视频**后 Alt+左键点它,以**当前帧**为源图再生成一段——可对自己生成的视频不断「续写」。跨域视频由后台抓取字节、经 `blob:` 抓帧,绕开 canvas 跨域污染。
- **任意图片来源**:普通网页图、跨域防盗链图(如各类 booru 站)、本地 `file://` 图片都支持。
- **穿透覆盖层**:对图片上盖了透明点击层的站点(如**抖音图文**),Alt+点击会用 `elementsFromPoint` 找到被盖住的真实 `<img>`;带 CORS 的跨域 CDN(如 douyinpic)由浏览器端 `fetch→重画`抓取。
...
- **视频关键帧**:**先把视频暂停**到想要的画面 → **Alt + 左键点击该视频** → 以当前帧为源图再生成。可对生成结果反复续写。
